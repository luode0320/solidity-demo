...
// 需要部署的合约名称
const contractName: string = "ReceiveETH";
...
main()
    .then((contractAddress) => {
        ...
        if (networkName == "localhost") {
            ...
            // 写入 HTML 文件
            fs.writeFile(path.join(path.dirname(__dirname), 'ReceiveETH.html'), htmlContent, err => {
                if (err) throw err;
                console.error("生成调试 html,请用 Live Server 调试:", path.join(path.dirname(__dirname), 'ReceiveETH.html'));
            });
        }
    })
    .catch(error => {
