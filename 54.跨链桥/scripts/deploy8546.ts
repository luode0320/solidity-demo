...
// 需要部署的合约名称
const name: string = "CrossChainToken";
// 合约的测试 html 名称
const contractName: string = name ? name : process.env.DEPLOY_CONTRACT_NAME!;
console.log("合约名称:", contractName);

// 合约的测试 html 名称
const contractHtml: string = name ? `CrossChainToken8546.html` : "index.html";
...
async function constructor(contractFactory: any) {
    ...
    const contract = await contractFactory.deploy("luode", 'ld', 10000);
    return contract;
}
